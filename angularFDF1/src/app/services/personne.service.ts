...
export class PersonneService {
  ...
  private WS_REST_BASE_URL = "http://localhost:8080/fdf1/personne-rest/personne";
  ...
  supprimerPersonne(personne: IPersonne): Observable<void> {
    return this.httpClient.delete<void>(`${this.WS_REST_BASE_URL}Delete/${personne.idPersonne}`).pipe(tap(() => { this.refreshNeeded.next() }));
  ...
  modifierPersonne(personne: IPersonne): Observable<void> {
    return this.httpClient.put<void>(`${this.WS_REST_BASE_URL}Update/${personne.idPersonne}`, personne).pipe(tap(() => { this.refreshNeeded.next() }));
